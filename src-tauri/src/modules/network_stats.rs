...
impl NetworkStats {
    ...
    /// Simulate bandwidth for demonstration purposes
    /// This can be used when real packet capture is not available
    pub async fn simulate_bandwidth(&self, mac: &str, ip: Ipv4Addr) {
        let mut stats = self.device_stats.lock().await;

        // Generate some realistic-looking bandwidth values
        let current_mbps = if let Some(prev) = stats.get(mac) {
            // Vary the bandwidth slightly from previous value
            let variation = (rand::random::<f64>() - 0.5) * 2.0; // +/- 1 MB/s
            (prev.bandwidth_mbps + variation).max(0.0).min(100.0)
        } else {
            // Initial random bandwidth between 0 and 10 MB/s
            rand::random::<f64>() * 10.0
        };

        stats.insert(mac.to_string(), DeviceBandwidth {
            ip,
            mac: mac.to_string(),
            bytes_sent: 0,
            bytes_received: 0,
            last_measurement: SystemTime::now(),
            bandwidth_mbps: current_mbps,
        });
    }
}
